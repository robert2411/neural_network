...
public class Net {
	...
	public void feedForward(Vector<Double> inputVals){
		...
		for (int i = 1; i < layers.size(); ++i){
			...
			for (int j = 0; j < layers.get(i).size() - 1 ; ++j){
				layers.get(i).get(j).feedForward(prevLayer);
			}
		}
	...
	public void backProp(Vector<Double> targetVals){
		...
		this.recentAverageError = (this.recentAverageError * Net.recentAverageSmoothingsFactor + error) / (Net.recentAverageSmoothingsFactor + 1.0);
	...
	static protected double recentAverageSmoothingsFactor = 100;
